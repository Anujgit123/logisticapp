...
<?php 
						if(isset($_GET['cid']))
						{
							...
							<form method="post" enctype="multipart/form-data">
									   ...
									   <?php $sale  = $lundry->query("select sum(o_total) as full_total from tbl_order where o_status='Completed'  and p_method_id=2 and  rid=".$_GET['cid']."")->fetch_assoc();
									   $sales  = $lundry->query("select sum(total) as full_total from tbl_logistics where status='Completed'  and p_method_id=2 and  rid=".$_GET['cid']."")->fetch_assoc();
									   $saless  = $lundry->query("select sum(total) as full_total from tbl_parcel where status='Completed'  and p_method_id=2 and  rid=".$_GET['cid']."")->fetch_assoc();
									   $pb = 0;
									   $tbl_order = empty($sale['full_total']) ? $pb : $sale['full_total'];
														 $tbl_logistics = empty($sales['full_total']) ? $pb : $sales['full_total'];
														 $tbl_parcel = empty($saless['full_total']) ? $pb : $saless['full_total'];
	...
	$pyout = empty($payout['full_payouts']) ? $vp : $payout['full_payouts'];
				 ...
				 $pb  = number_format((float)($tbl_order + $tbl_logistics + $tbl_parcel) - $pyout, 2, '.', ''); } ?>
